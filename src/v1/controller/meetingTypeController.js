...
const getAllMeetingTypes = async (req, res, next) => {
    try {
        ...
        const callStatuses = await meetingTypeService.getAllMeetingTypes(is_active,search ,Number(page), Number(size),startDate && moment(startDate),endDate && moment(endDate));
