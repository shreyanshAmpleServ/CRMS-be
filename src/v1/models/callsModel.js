...
const createCalls = async (data, user) => {
  try {
    ...
    const { assign_to_name, assign_to_email, ...datas } = data;
    const calls = await prisma.crms_calls.create({
      data: {
        ...datas,
        createdate: new Date(),
        call_start_date: combineDateAndTime(
          datas.call_start_date,
          datas.call_start_time,
        ),
        updatedate: new Date(),
        createdby: user?.id || 1,
    ...
    (async () => {
      try {
        if (
          ...
          datas?.ongoing_callStatus === "Scheduled" &&
