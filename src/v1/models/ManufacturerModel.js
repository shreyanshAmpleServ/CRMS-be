...
const findManufacturerById = async (id) => {
    try {
        ...
        throw new CustomError(`Error finding manufacturer by ID: ${error.message}`, 503);
    }
};
    ...
    try {
        ...
        if (error instanceof PrismaClientKnownRequestError) {
            if (error.code === "P2003") {
              ...
              throw new Error(
                "Cannot delete this manufacturer because related records exist. Please remove them first."
              );
            }
            if (error.code === "P2025") {
              // Record not found
