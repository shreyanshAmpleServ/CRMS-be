  ...
  try {
    ...
    if (existing) {
      throw new CustomError(
        `Currency already exists with same name or code (Name: ${data.name}, Code: ${data.code})`,
        400,
      );
    }
...
const updateCurrency = async (id, data) => {
  try {
    ...
    if (existing) {
      throw new CustomError(
        `Another currency already exists with same name or code`,
        400,
      );
    }
    ...
    if (data.is_default === "Y") {
      await prisma.Currency.updateMany({
        where: {
          is_default: "Y",
          NOT: { id: parseInt(id) }, // 🔥 avoid resetting itself
        },
        data: { is_default: "N" },
      });
    }
