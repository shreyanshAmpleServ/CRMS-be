...
const deleteUser = async (id) => {
    try {
        ...
        await prisma.crms_d_user_role.deleteMany({ where: { user_id: parseInt(id) } });
...
const getAllUsers = async (search,page , size ,startDate, endDate) => {
    try {
        ...
        const usersList =  await prisma.crms_m_user.findMany({
            where: filters,
            skip,
            take: size,
            ...
            select: {
                ...getUserFields(),
                crms_d_user_role: { select: { crms_m_role: { select: { role_name: true, id: true } } } },
            },
        });
        const totalCount = await prisma.crms_m_user.count({
            where: filters,
        });
