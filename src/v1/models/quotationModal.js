...
const createQuotation = async (orderData, orderItemsData) => {
  try {
    ...
    const result = await prisma.$transaction(async (prisma) => {
      ...
      const quotationCode = `QUO-${formattedId}`;

      // ✅ Step 3: Update code
      await prisma.crms_d_quotations.update({
        where: { id: createdOrder.id },
        data: { quotation_code: quotationCode }, // 👈 your column
      });
...
const generateQuotaionCode = async () => {
  try {
    ...
    // 🔥 Ensure minimum 4 digits with leading zeros
    const formattedId = String(nextId).padStart(4, "0");

    return `QUO-${formattedId}`;
  } catch (error) {
